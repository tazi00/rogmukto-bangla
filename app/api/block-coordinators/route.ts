...
export async function GET(req: NextRequest) {
  ...
  const data = await BlockCoordinator.find().select('-password').sort({ createdAt: -1 })
...
export async function POST(req: NextRequest) {
  ...
  const userExists = await BlockCoordinator.findOne({ username: body.username })
  if (userExists) return NextResponse.json({ error: 'Username already taken' }, { status: 400 })
  const hashed = await bcrypt.hash(body.password, 10)
  const bc = await BlockCoordinator.create({ ...body, password: hashed })
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await BlockCoordinator.findByIdAndUpdate(id, update, { new: true }).select('-password')
