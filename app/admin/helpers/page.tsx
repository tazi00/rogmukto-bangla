...
const EMPTY_FORM = { name: '', phone: '', tag: 'Swasthya Bondhu' }
...
export default function HelpersPage() {
  ...
  const sdData = locations.find(sd => sd.name === selectedBC?.subDivision)
  ...
  function openAdd() {
    ...
    setSelectedBlock(''); setUseGP(false); setUseMun(false)
    setSelectedGPs([]); setSelectedMuns([]); setError(''); setShowModal(true)
  }

  function openEdit(h: Helper) {
    setEditItem(h)
    setForm({ name: h.name, phone: h.phone, tag: h.tag })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (useGP && selectedGPs.length === 0) { setError('Select at least one Gram Panchayat'); return }
    if (useMun && selectedMuns.length === 0) { setError('Select at least one Municipality'); return }
    setLoading(true); setError('')
    try {
      const body = {
        ...form,
        blockCoordinatorId: selectedBC._id,
        subDivision: selectedBC.subDivision,
        block: selectedBlock,
                ...
                {selectedBC && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div className="form-group">
                      ...
                      <input className="form-input" value={selectedBC.subDivision} disabled style={{ opacity: 0.6 }} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Block *</label>
                      {selectedBC.blocks.length === 1
                        ? <input className="form-input" value={selectedBC.blocks[0]} disabled style={{ opacity: 0.6 }} />
                        : <SearchableSelect
                            options={selectedBC.blocks.map(b => ({ label: b, value: b }))}
                            value={selectedBlock}
                            onChange={v => { setSelectedBlock(v); setSelectedGPs([]); setSelectedMuns([]) }}
                  ...
                  <>
                    <div className="form-group">
                      ...
                      <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
                        {[{ key: 'gp', label: '🌿 Gram Panchayat', active: useGP, toggle: () => { setUseGP(!useGP); setSelectedGPs([]) } },
                          { key: 'mun', label: '🏙 Municipality', active: useMun, toggle: () => { setUseMun(!useMun); setSelectedMuns([]) } }
                        ].map(item => (
                          <label key={item.key} onClick={item.toggle} style={{
                            display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px',
                            border: `2px solid ${item.active ? 'var(--green-mid)' : 'var(--border)'}`,
                            borderRadius: 'var(--radius-sm)',
                            background: item.active ? 'var(--green-light)' : 'var(--surface)',
                    ...
                    {useGP && blockData && (
                      <div className="form-group">
                        ...
                        {selectedGPs.map(selGP => {
                          const gpObj = blockData.gramPanchayats.find(g => g.name === selGP.gpName)
                          ...
                          return (
                            <div key={selGP.gpName} style={{ marginTop: 8, paddingLeft: 12, borderLeft: '3px solid var(--green-light)' }}>
                              <MultiSearchSelect
                                ...
                                options={gpObj.villages.map(v => ({ label: v.name, value: v.name }))}
                                values={selGP.villages}
                                onChange={villages => setGPVillages(selGP.gpName, villages)}
                    ...
                    {useMun && blockData && (
                      <div className="form-group">
                        ...
                        {selectedMuns.map(selMun => {
                          const munObj = blockData.municipalities.find(m => m.name === selMun.municipalityName)
                          ...
                          return (
                            <div key={selMun.municipalityName} style={{ marginTop: 8, paddingLeft: 12, borderLeft: '3px solid #d0d8ff' }}>
                              <MultiSearchSelect
                                ...
                                options={munObj.wards.map(w => ({ label: w.name, value: w.name }))}
                                values={selMun.wards}
                                onChange={wards => setMunWards(selMun.municipalityName, wards)}
