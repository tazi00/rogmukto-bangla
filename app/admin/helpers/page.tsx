...
export default function HelpersPage() {
  ...
  const [surveyEditHealthIssues, setSurveyEditHealthIssues] = useState<
    SurveyHealthIssue[]
  >([]);
  ...
  async function openSurveyModal(h: Helper) {
    ...
    const data = await fetch(`/api/surveys?sbId=${h._id}`).then((r) =>
      r.json(),
    );
  ...
  function openEdit(h: Helper) {
    ...
    setForm({
      ...
      phone: h.phone,
      tag: h.tag,
      doj: h.doj ? new Date(h.doj).toISOString().slice(0, 10) : TODAY_ISO,
    });
  ...
  async function createSD(name: string) {
    ...
    const sd = Array.isArray(data)
      ? data.find((s: SubDiv) => s.name === name)
      : null;
    if (sd) {
      setSelectedSDId(sd._id);
      setSelectedBlock("");
  ...
  const pagedFiltered = filtered.slice((page - 1) * 5, page * 5);
          ...
          <table style={{ fontSize: 14, minWidth: 1300 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                pagedFiltered.map((h) => {
                  const gpNames =
                    h.gramPanchayats?.map((g) => `🌿 ${g.gpName}`) || [];
                  const munNames =
                    h.municipalities?.map((m) => `🏙 ${m.municipalityName}`) ||
                    [];
                  const allGPMun = [...gpNames, ...munNames];
                  const villageNames =
                    h.gramPanchayats?.flatMap((g) =>
                      g.villages.map((v) => `🏘 ${v}`),
                    ) || [];
                  const wardNames =
                    h.municipalities?.flatMap((m) =>
                      m.wards.map((w) => `🔢 ${w}`),
                    ) || [];
                  ...
                  return (
                    <tr key={h._id}>
                      ...
                      <td style={{ ...tdStyle, whiteSpace: "nowrap" }}>
                        {h.doj
                          ? new Date(h.doj).toLocaleDateString("en-IN", {
                              day: "2-digit",
                              month: "short",
                              year: "numeric",
                            })
                      ...
                      <td style={{ ...tdStyle, textAlign: "center" }}>
                        {(surveyCounts[h._id] ?? 0) === 0 ? (
                          ...
                          >
                            📋 {surveyCounts[h._id]}{" "}
                            {surveyCounts[h._id] === 1 ? "family" : "families"}
                          </button>
                        )}
                      </td>
                      ...
                      <td style={{ ...tdStyle, textAlign: "center" }}>
                        <span
                          style={{
                            ...
                            borderRadius: "50%",
                            background:
                              (patientCounts[h._id] ?? 0) > 0
                                ? "var(--green-light, #d8f3dc)"
                                : "var(--surface)",
                        ...
                        {selectedGP && (
                          ...
                          >
                            ...
                            <CreatableSearchSelect
                              ...
                              onCreateOption={(vName) =>
                                createVillage(selectedGP, vName)
                              }
                              creating={creating}
                      ...
                      <div className="form-group">
                        ...
                        {selectedMun && (
                          ...
                          >
                            ...
                            <CreatableSearchSelect
                              options={(
                                blockData.municipalities.find(
                                  (m) => m.name === selectedMun,
                                )?.wards || []
                              )
                              ...
                              onCreateOption={(wName) =>
                                createWard(selectedMun, wName)
                              }
                              creating={creating}
            ...
            >
              <div>
                <div style={{ fontSize: 17, fontWeight: 700 }}>
                  Survey Report — {surveyModalSB.name}
                </div>
                <div
                  style={{
                    fontSize: 13,
                ...
                >
                  {surveyModalSB.helperId || "—"} · {surveyModalSB.subDivision}{" "}
                  · {surveyModalSB.block}
                </div>
              </div>
              <button
            ...
            {surveyLoading ? (
              ...
              <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
                ...
                >
                  ...
                  >
                    {surveyList.length}{" "}
                    {surveyList.length === 1 ? "Family" : "Families"}
                  </div>
                  {surveyList.map((s) => (
                    <div
                      key={s._id}
                      ...
                      {s.healthIssueDetected && s.healthIssues.length > 0 && (
                        ...
                        >
                          {s.healthIssues.slice(0, 2).map((hi, i) => {
                            const cfg =
                              HEALTH_CONFIG[hi.type] || HEALTH_CONFIG.others;
                            return (
                              <span
                                key={i}
                                style={{
                  ...
                  {!selectedSurvey ? (
                    ...
                    <div>
                      ...
                      <div className="form-group">
                        ...
                        <input
                          className="form-input"
                          value={surveyEditForm.familyName}
                          onChange={(e) =>
                            setSurveyEditForm({
                              ...surveyEditForm,
                      ...
                      >
                        <div className="form-group">
                          ...
                          <input
                            className="form-input"
                            maxLength={10}
                            value={(surveyEditForm as any).mobile || ""}
                            onChange={(e) =>
                              setSurveyEditForm({
                                ...surveyEditForm,
                        ...
                        <div className="form-group">
                          ...
                          <input
                            className="form-input"
                            maxLength={10}
                            value={(surveyEditForm as any).whatsapp || ""}
                            onChange={(e) =>
                              setSurveyEditForm({
                                ...surveyEditForm,
                      ...
                      <div style={{ marginBottom: 12 }}>
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={surveyEditForm.healthIssueDetected}
                            onChange={(e) => {
                              setSurveyEditForm({
                                ...surveyEditForm,
                                healthIssueDetected: e.target.checked,
                              });
                              if (!e.target.checked)
                                setSurveyEditHealthIssues([]);
                            }}
                          />
                      ...
                      <div style={{ display: "flex", gap: 10 }}>
                        ...
                        <button
                          disabled={surveyEditLoading}
                          className="btn btn-primary"
                          onClick={async () => {
                            setSurveyEditLoading(true);
                            setSurveyEditError("");
                            try {
                              const res = await fetch(
                                `/api/surveys?id=${editingSurvey._id}`,
                                {
                                  method: "PUT",
                                  headers: {
                              ...
                              if (!res.ok) {
                                const d = await res.json();
                                setSurveyEditError(d.error || "Failed");
                                return;
                              }
                              const data = await fetch(
                                `/api/surveys?sbId=${surveyModalSB!._id}`,
                              ).then((r) => r.json());
                              const list = Array.isArray(data) ? data : [];
                              setSurveyList(list);
                              setSelectedSurvey(
                                list.find(
                                  (s: Survey) => s._id === editingSurvey._id,
                                ) || null,
                              );
                              setEditingSurvey(null);
                            } catch {
                              setSurveyEditError("Something went wrong");
                            } finally {
                              setSurveyEditLoading(false);
                            }
                          }}
                        >
                          {surveyEditLoading ? "Saving..." : "Save Changes"}
                        </button>
                      </div>
                    </div>
                  ) : (
                    ...
                    <>
                      ...
                      >
                        <div style={{ fontSize: 20, fontWeight: 700 }}>
                          {selectedSurvey.familyName} Family
                        </div>
                        <button
                          className="btn btn-secondary btn-sm"
                          onClick={() => {
                            ...
                            setSurveyEditForm({
                              ...
                              membersM: selectedSurvey.membersM,
                              membersF: selectedSurvey.membersF,
                              childM: selectedSurvey.childM,
                              childF: selectedSurvey.childF,
                              above65M: selectedSurvey.above65M,
                              above65F: selectedSurvey.above65F,
                              healthIssueDetected:
                                selectedSurvey.healthIssueDetected,
                            } as any);
                            setSurveyEditHealthIssues(
                              selectedSurvey.healthIssues.map((h) => ({
                                ...h,
                              })),
                            );
                            setSurveyEditError("");
                      ...
                      >
                        {selectedSurvey.village || selectedSurvey.ward || "—"} ·
                        Surveyed on{" "}
                      ...
                      {(selectedSurvey.mobile || selectedSurvey.whatsapp) && (
                        ...
                        >
                          {selectedSurvey.mobile && (
                            <span>📞 {selectedSurvey.mobile}</span>
                          )}
                          {selectedSurvey.whatsapp && (
                            <span>💬 {selectedSurvey.whatsapp}</span>
                          )}
                        </div>
                      )}
                      <div
                      ...
                      >
                        ...
                        >
                          👥 Total Members:{" "}
                          {selectedSurvey.membersM +
                            selectedSurvey.membersF +
                            selectedSurvey.childM +
                            selectedSurvey.childF +
                            selectedSurvey.above65M +
                            selectedSurvey.above65F}
                        </span>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        {[
                          {
                            label: "Members",
                            m: selectedSurvey.membersM,
                            f: selectedSurvey.membersF,
                          },
                          {
                            label: "Children",
                            m: selectedSurvey.childM,
                            f: selectedSurvey.childF,
                          },
                          {
                            label: "Above 65",
                            m: selectedSurvey.above65M,
                            f: selectedSurvey.above65F,
                          },
                        ].map(({ label, m, f }) => (
                          <div
                            key={label}
                      ...
                      {selectedSurvey.healthIssueDetected &&
                      selectedSurvey.healthIssues.length > 0 ? (
                        <>
                          <div
                            style={{
                              fontSize: 14,
