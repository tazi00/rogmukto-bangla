...
interface BC { _id: string; coordinatorId: string; name: string; phone: string; subDivision: string; blocks: string[]; address: string; username: string }
interface SubDiv { _id: string; name: string; blocks: { _id: string; name: string }[] }

const EMPTY = { coordinatorId: '', name: '', phone: '', subDivision: '', blocks: [] as string[], address: '', username: '', password: '' }
...
export default function BlockCoordinatorsPage() {
  ...
  function openEdit(bc: BC) {
    setEditItem(bc)
    setForm({ coordinatorId: bc.coordinatorId, name: bc.name, phone: bc.phone, subDivision: bc.subDivision, blocks: bc.blocks, address: bc.address, username: bc.username, password: '' })
    ...
    <>
      ...
      <div className="page-body">
        <div className="table-wrapper">
          <table>
            <thead>
              <tr><th>ID</th><th>Name</th><th>Phone</th><th>Username</th><th>Sub Division</th><th>Blocks</th><th>Address</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {list.length === 0 ? (
                <tr><td colSpan={8}><div className="empty-state"><p>No block coordinators yet.</p></div></td></tr>
              ) : list.map(bc => (
                <tr key={bc._id}>
                  ...
                  <td><span style={{ fontFamily: 'monospace', fontSize: 12, background: 'var(--gray-100)', padding: '2px 8px', borderRadius: 4 }}>{bc.username}</span></td>
              ...
              <div className="modal-body">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div className="form-group">
                    ...
                    <input className="form-input" required value={form.coordinatorId} onChange={e => setForm({...form, coordinatorId: e.target.value})} placeholder="e.g. BC-001" disabled={!!editItem} />
                  </div>
                  <div className="form-group">
                  ...
                  <div className="form-group">
                    ...
                    <SearchableSelect options={sdOptions} value={form.subDivision}
                      onChange={v => setForm({...form, subDivision: v, blocks: []})} placeholder="Select Sub Division" />
                  </div>
                  <div className="form-group">
                  ...
                  <div className="form-group">
                    <label className="form-label">{editItem ? 'New Password (blank = no change)' : 'Password *'}</label>
                ...
                <div className="form-group">
                  <MultiSearchSelect label="Blocks * (multiple select)" options={blockOptions} values={form.blocks}
                ...
                <div className="form-group">
                  ...
                  <textarea className="form-input" rows={2} value={form.address}
                    ...
                    placeholder="Full address" style={{ resize: 'vertical', fontFamily: 'inherit' }} />
                </div>
              </div>
