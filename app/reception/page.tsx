...
export default function ReceptionPage() {
  ...
  function openAdd() {
    ...
    setSelectedHelper(null); setHelperSearch("");
    setError(""); setShowForm(true);
  }

  function openEdit(p: Patient) {
    setEditPatient(p);
    const h = helpers.find(h => h._id === (p.helperId as any)?._id || h._id === (p.helperId as any));
  ...
  const displayPatients = filterHelper
    ? patients.filter(p => (p.helperId as any)?._id === filterHelper || p.helperId === filterHelper as any)
    : patients;
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            {filterHelper && <button className="btn btn-secondary btn-sm" onClick={() => setFilterHelper("")}>✕ Clear</button>}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-secondary" onClick={() => setShowAddHelper(true)}>+ Add Swasthya Bondhu</button>
          ...
          <table>
            ...
            <tbody>
              {displayPatients.length === 0 ? (
                <tr><td colSpan={9}><div className="empty-state" style={{ padding: "24px" }}><p>No patients found.</p></div></td></tr>
                ...
                <tr key={p._id}>
                  ...
                  <td style={{ fontSize: 12 }}>{(p.helperId as any)?.name || "—"}</td>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                ...
                <div className="form-group" ref={dropRef} style={{ position: "relative" }}>
                        ...
                        : filteredHelpers.map(h => (
                          ...
                          >
                            ...
                            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                              📞 {h.phone} · 📍 {h.block}
                              {h.gramPanchayats?.length > 0 && ` · 🌿 ${h.gramPanchayats.map(g => g.gpName).join(", ")}`}
                              {h.blockCoordinatorId && ` · ID: ${h.blockCoordinatorId.coordinatorId}`}
                              <span className="badge badge-green" style={{ marginLeft: 6, fontSize: 10 }}>{h.tag}</span>
                            </div>
                          </div>
                        ))
                      }
                  ...
                  {selectedHelper && (
                    <div style={{ marginTop: 6, padding: "8px 12px", background: "var(--green-light)", borderRadius: "var(--radius-sm)", fontSize: 12, color: "var(--green-dark)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span>✓ <strong>{selectedHelper.name}</strong> · {selectedHelper.block} · 📞 {selectedHelper.phone}</span>
                      <button type="button" style={{ background: "none", border: "none", cursor: "pointer", color: "var(--green-dark)", fontSize: 14 }}
                        onClick={() => { setSelectedHelper(null); setHelperSearch(""); setForm(f => ({ ...f, helperId: "" })); setShowHelperDrop(true); }}>✕</button>
                    </div>
                  )}
                </div>
