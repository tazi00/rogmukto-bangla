...
interface Props {
  ...
  onCreateOption?: (val: string) => Promise<void>
  placeholder?: string
  disabled?: boolean
  label?: string
  creating?: boolean
...
export default function CreatableSearchSelect({ options, value, onChange, onCreateOption, placeholder = 'Select or type to add...', disabled, label, creating }: Props) {
  ...
  const canCreate = onCreateOption && search.trim() && !options.find(o => o.label.toLowerCase() === search.trim().toLowerCase())
    ...
    <div ref={ref} style={{ position: 'relative', width: '100%' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
          {selected && !disabled && (
            <span onClick={clear} style={{ fontSize: 12, color: 'var(--text-muted)', cursor: 'pointer', padding: '0 2px' }}>✕</span>
          )}
          ...
          <div style={{ maxHeight: 200, overflowY: 'auto' }}>
            ...
            {canCreate && (
              <div
                ...
                style={{
                  padding: '9px 14px', cursor: creating ? 'not-allowed' : 'pointer', fontSize: 13,
