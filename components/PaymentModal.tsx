...
export default function PaymentModal({ patient, onClose, onSave }: Props) {
  ...
  const [denoms, setDenoms] = useState<Record<number, number>>(
    Object.fromEntries(
      (patient.paymentDetail?.denominations || []).map((d: any) => [
        d.note,
        d.count,
      ]),
    ),
  ...
  const cashTotal = Object.entries(denoms).reduce(
    (s, [note, count]) => s + Number(note) * count,
    0,
  );
    ...
    await fetch("/api/payment", {
      ...
      body: JSON.stringify({
        ...
        paymentDetail: {
          mode,
          remarks,
          denominations: mode === "cash" ? denomArr : [],
        },
      }),
    });
    setLoading(false);
          ...
          <div className="form-group">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[
                ...
                { value: "clearance", label: "✓ Cleared", color: "#1a7a4a" },
              ].map((opt) => (
                <button
                  key={opt.value}
                  type="button"
            ...
            >
              ...
              >
                {DENOMINATIONS.map((note) => (
                  ...
                  >
                    <span style={{ fontSize: 13, fontWeight: 500, width: 48 }}>
                      ₹{note}
                    </span>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontWeight: 700,
                    color:
                      cashTotal === patient.incentiveAmount
                        ? "var(--green)"
                        : cashTotal > 0
                          ? "var(--red)"
                          : "var(--text)",
                ...
                >
                  ...
                  {cashTotal > 0 && cashTotal !== patient.incentiveAmount && (
                    ...
                    >
                      (expected ₹{patient.incentiveAmount})
                    </span>
                  )}
                </span>
              </div>
